...
class BayesianNet(nn.Module):
    ...
    def print_net(self):
        ...
        for i in range(self.n_layers):
            print("The mean of the weights ", mu_w[i])
            print("The mean of the biases ", mu_b[i])
            print("The standard deviation of the weights ",rho_w[i])
            print("The standard deviation of the biases ",rho_b[i])
    ...
    def create_image(self):
        ...
        return np.concatenate((img_mu,img_rho),axis = 0)
    
    def draw_weight_bias(self, weights, biases):
        '''
            Expects an list of the weights and the biases 
        '''
        lcm = int(np.lcm.reduce(self.arc))
        bias_width = int(np.ceil(0.2*lcm))
        single_width = lcm + bias_width
        height = lcm
        width = int((self.n_layers-1)*single_width)
        img = np.zeros((height,width))
        for i in range(self.n_layers-1):
            img[:,i*single_width:(i+1)*single_width] = self.draw_weight_bias_block(weights[i],biases[i],bias_width)
        return img

    def draw_weight_bias_block(self, weight, bias,bias_width):
        lcm = np.lcm.reduce(self.arc)
        ...
        longer_bias = np.repeat(longer_bias, bias_width, axis = 1)
